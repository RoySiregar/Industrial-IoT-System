            ...
            <div class="flex gap-4">
              <div class="flex flex-wrap items-center gap-2"> 
                <div v-if="activeFilters.includes('line')">
                  <select v-model="filters.line" @change="fetchOeeData" class="select select-bordered select-xs w-20">
                ...
                <div v-if="activeFilters.includes('date')" class="flex items-center gap-2">
                  <input type="datetime-local" v-model="filters.dateFrom" class="input input-bordered input-xs" />
                  <span class="strip">-</span>
                  <input type="datetime-local" v-model="filters.dateUntil" class="input input-bordered input-xs" />
                  <button @click="fetchOeeData" class="btn btn-xs btn-accent">{{$t('Apply')}}</button>
                </div>
              </div>
            </div>
          </div>
...
export default {
  ...
  mounted() {
    const now = new Date();
    this.filters.dateFrom = this.formatDateTimeLocal(new Date(now.getFullYear(), now.getMonth(), now.getDate(), 0, 0, 0));
    this.filters.dateUntil = this.formatDateTimeLocal(new Date(now.getFullYear(), now.getMonth(), now.getDate(), 23, 59, 59));
    this.fetchOeeData();
  },
  methods: {
    ...
    async fetchOeeData() {
      ...
      const apiUrl = `http://10.176.33.76:6002/api/OeeByWorkshifts/calculate?${params.toString()}&${cacheBuster}`;
